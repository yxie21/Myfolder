...
public class pdfreport {
    ...
    // main测试
    public void pdfgenerator(File[]list) throws Exception {
        try {
            // 1.新建document对象
            Document document = new Document(PageSize.A4);// 建立一个Document对象
 
            // 2.建立一个书写器(Writer)与document对象关联
            File file = new File("/Users/yuxinxie/Desktop/test.pdf");
            file.createNewFile();
    ...
    // 生成PDF文件
	public void generatePDF(File[] list, Document document) throws Exception {
			...
			Paragraph time = new Paragraph(name);
			
			
			
			Paragraph p4= new Paragraph ();
			
			time.setAlignment(1); //设置文字居中 0靠左   1，居中     2，靠右
			time.setIndentationLeft(12); //设置左缩进
			time.setIndentationRight(12); //设置右缩进
			time.setFirstLineIndent(24); //设置首行缩进
			time.setLeading(20f); //行间距
			time.setSpacingBefore(5f); //设置段落上空白
			time.setSpacingAfter(10f); //设置段落下空白
			time.setFont(headfont);
			document.add(time);
			Paragraph dot = new Paragraph();
			dot.add(new Chunk(new DottedLineSeparator()));
			document.add(dot);
			...
			for (File folders : list) {
				if(!folders.isDirectory()) {
					...
					Paragraph p1 = new Paragraph(folders.getName());
					p1.setAlignment(1); //设置文字居中 0靠左   1，居中     2，靠右
					...
					Paragraph p2 = new Paragraph("Folder"+" "+ folders.getName());
					p2.setAlignment(1); //设置文字居中 0靠左   1，居中     2，靠右
					p2.setIndentationLeft(12); //设置左缩进
					p2.setIndentationRight(12); //设置右缩进
					p2.setFirstLineIndent(24); //设置首行缩进
					p2.setLeading(20f); //行间距
					...
					for (File subfolder : folders.listFiles()) {
						if (!subfolder.isDirectory()) {
							...
							Paragraph p3 = new Paragraph(subfolder.getName());
							p3.setAlignment(1); //设置文字居中 0靠左   1，居中     2，靠右
							p3.setIndentationLeft(12); //设置左缩进
							p3.setIndentationRight(12); //设置右缩进
							p3.setFirstLineIndent(24); //设置首行缩进
							p3.setLeading(20f); //行间距
							...
							Paragraph sub= new Paragraph("Subfolder" + "  " + subfolder.getName());
							sub.setAlignment(1); //设置文字居中 0靠左   1，居中     2，靠右
							sub.setIndentationLeft(12); //设置左缩进
							sub.setIndentationRight(12); //设置右缩进
							sub.setFirstLineIndent(24); //设置首行缩进
							sub.setLeading(20f); //行间距
						sub.setSpacingBefore(5f); //设置段落上空白
							sub.setSpacingAfter(10f); //设置段落下空白
							document.add(sub);
							...
							for (File source : subfolder.listFiles()) {
								...
								p5.add(source.getName());
								p5.setAlignment(1); //设置文字居中 0靠左   1，居中     2，靠右
								p5.setIndentationLeft(12); //设置左缩进
								p5.setIndentationRight(12); //设置右缩进
								p5.setFirstLineIndent(24); //设置首行缩进
								p5.setLeading(20f); //行间距
							p5.setSpacingBefore(5f); //设置段落上空白
								p5.setSpacingAfter(10f); //设置段落下空白
								document.add(p5);
